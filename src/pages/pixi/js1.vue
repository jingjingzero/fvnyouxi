<template>
  <div class="absolute z-999 text-black">
    x:{{ user.ceshi1.toFixed(0) }} y:{{ user.ceshi2.toFixed(0) }} 地面:{{
      user.pixi.isPaused
    }}
  </div>
...
//   wall = new TilingSprite(wallTexture, WORLD_WIDTH, 90 * VH()); // 宽 750*2，高 750
//   wall.roundPixels = true;
//   wall.tileScale.set(0.5);
//   wall.x = 0;
//   wall.y = 0;
...
function BgWall() {
  WallTexture = Texture.from("wall");
  // 1️⃣ 按“屏幕高度”等比缩放
  WallScale = DEVICE_HEIGHT / DESIGN_BG_HEIGHT;
  // 3️⃣ 宽度不限制（先给一个足够大的值）
  BgWidthPx = WallTexture.width * WallScale;
}
function createWallObject() {
  // 2️⃣ 真实像素高度 = 屏幕高度
  const heightPx = WallTexture.height * WallScale;
  wall = new TilingSprite(WallTexture, BgWidthPx, heightPx);
  // 4️⃣ 只控制 tileScale，保持清晰
  wall.tileScale.set(WallScale); // 等比缩放，x=y
  // 5️⃣ 左下角对齐（底部贴屏幕）
  wall.x = 0;
  wall.y = DEVICE_HEIGHT - heightPx;
  wall.roundPixels = true;
  bgContainer.addChild(wall);
}
...
function createTeshu(x, y, w, h, { texture = null, name = false,show=true} = {}) {
  if (!show) {
    return
  }
  const options = {
  ...
  spine.view.scale.set(WallScale); // 缩小到50%
  spine.view.y = y;
  spine.view.x = x;
  worldContainer.addChild(spine.view);
  return spine
...
function createRectFromData(rectData, index, name) {
  ...
  if (name === "矩形") {
    ...
    console.log('teshuSpine=',teshuSpine);
    return
  }
