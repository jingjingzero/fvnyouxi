 ...
 * @LastEditTime: 2025-04-22 15:00:22
-->
<template>
  <div class="flex w-full h-full flex-col">
    ...
    <!-- 文字描述 -->
    <div v-show="!skillShow" class="w-60vw h-22vh absolute bottom-2vh left-55% -translate-x-1/2 box-border pt-2vh px-2vw text-white bg-[rgba(0,0,0,0.6)] typewriter-text">
      <!-- 文字 -->
...
function direnhuihe(item) {
  ...
  direnTexiao({ x: centerXvw, y: centerYvh }, imgEl1, index1, index2, { diren: List[random].name, wo: item.name, type: true });
}
// 特效
...
function shanghai(duixiang, index2, imgEl) {
  ...
  if (duixiang.type) {
    ...
    if (duixiang.diren.name === user.attributes.myName) {
      user.attributes.currentHp -= jineng.shanghai;
      if (user.attributes.currentHp < 0) {
...
function fightOver(params) {
          clearInterval(timer); // 停止定时器
  console.log("fightReward=", user.fightReward);
  if (!params) {
    //失败
    user.youxi += user.fightReward.shibai.youxi;
    user.attributes.exp += user.fightReward.shibai.exp;
  } else {
    for (let i = 0; i < user.fightReward.shengli.reward.length; i++) {
      user.addItemToInventory(user.fightReward.shengli.reward[i]);
    }
    user.youxi += user.fightReward.shengli.youxi;
    user.attributes.exp += user.fightReward.shengli.exp;
  }
